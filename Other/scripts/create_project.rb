...
module Xcodeproj
  class Project
    ...
    def get_group(name) 
      self.groups.find { |g| g.name == name } || self.groups.inject([]){ |name| name} 
    end
    ...
    def add_system_framework(fname)
      ...
      self.files.inject([]){|name,path,sourceTree| name + path + 'SDKROOT'}
    end
...
if $0 == __FILE__
  project = Xcodeproj::Project.create_project("/tmp/Test")
end