# TODO : 
# 1.Settings bundle
# 2.Build settings for different configurations
	...
	class Project
		...
		def self.add_files_to_group(currentGroup, path, applicationTarget)
			Dir.glob(path + '/*') do |file|
				if File.directory?(file) and !(file.include? ".xcassets")
					...
					elsif file.include? ".xcassets"
						applicationTarget.resources_build_phase.add_file_reference(ref)
					end
				end
			end
		end
